...
class CustomerCore extends ObjectModel
{
    ...
    public function add($autoDate = true, $nullValues = true)
    {
        ...
        if ($success){
            $id_customer = Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
                'SELECT MAX(c.`id_customer`) ' .
                'FROM `' . _DB_PREFIX_ . 'customer` c '
            );

            $query = "SELECT * FROM clientes WHERE mail = '$mail' LIMIT 1";
            ...
            if ($val){
                if (mysqli_num_rows($val) === 0){
                    ...
                    if (!mysqli_query($con, $query)){
                        throw new PrestaShopDatabaseException('Error al guardar el usuario en el sistema de Control');
                    }
                }
            }
        //MODIFICADO POR ALAN FIN
        }
