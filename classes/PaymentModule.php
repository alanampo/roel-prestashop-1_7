        ...
        if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false) {
            ...
            foreach ($order_detail_list as $key => $order_detail) {
                ...
                if (isset($order->id)) {
                    ...
                    $errors = "";
                    $hayProductoDeStock = false;
                    $id_customer = $order->id_customer;
                    $email = $this->context->customer->email;
                    $nombre_cliente = strtoupper($this->context->customer->firstname)." ".strtoupper($this->context->customer->lastname);
                    $query = "SELECT id_cliente FROM clientes WHERE id_customer_tienda = $id_customer OR mail = '$email'";
                    $valu = mysqli_query($con, $query);

                    if ($valu){
                        mysqli_autocommit($con, FALSE);
                        
                        $id_cliente_control = NULL;
                        if (mysqli_num_rows($valu) > 0){ //EL CLIENTE EXISTE EN LA DB
                            $id_cliente_control = mysqli_fetch_assoc($valu)["id_cliente"];
                        }
                        else{ //EL CLIENTE NO EXISTE - HAY QUE CREARLO
                            ...
                            if (!mysqli_query($con, $query)) {
                                ...
                                $id_cliente_control = mysqli_insert_id($con);
                            }
                        }

                        if (!isset($id_cliente_control)){
                            $error = $this->trans('Order creation failed - ERROR AL CREAR EL CLIENTE EN ROEL-CONTROL', [], 'Admin.Payment.Notification');
                            mysqli_close($con);
                        ...
                        foreach ($product_list as $product) {
                            ...
                            $ventaDesdeStock = (int) Db::getInstance()->getValue(
                                '
                                SELECT COUNT(*)
                            ...
                            if ($ventaDesdeStock > 0){ //ES UN PRODUCTO DE STOCK
                                //AGREGAR RESERVA EN SISTEMA DE CONTROL
                                $hayProductoDeStock = true;

                                
                                $codigo = "";
                                if ($product['reference'][0] == "E"){
                                    $codigo = "E";
                                }
                                else if ($product['reference'][0] == "S"){
                                    $codigo = "S";
                                }
                                else{
                                    $error = $this->trans('Order creation failed - LA REFERENCIA DEL PRODUCTO ES INCORRECTA', [], 'Admin.Payment.Notification');
                                    PrestaShopLogger::addLog($error, 4, '0000002', 'Cart', (int) ($order->id_cart));
                                    mysqli_close($con);
                                    die(Tools::displayError($error));
                                }

                                $id_interno = filter_var($product['reference'], FILTER_SANITIZE_NUMBER_INT);
                                ...
                                $query = "INSERT INTO reservas_productos (
                                    ...
                                    $cantidad,
                                    NOW(),
                                    ( SELECT * FROM (SELECT v.id FROM variedades_producto v
                                    INNER JOIN tipos_producto t ON t.id = v.id_tipo
                                    WHERE t.codigo = '$codigo' AND v.id_interno = '$id_interno'
                                    ) t1 ),
                                    $id_cliente_control,
                                    '',
                                    0,
                        ...
                        if ($hayProductoDeStock === true){
                            if (strlen($errors) === 0) {
                                if (!mysqli_commit($con)) {
                                    mysqli_rollback($con);
                                    $error = $this->trans('Order creation failed - ERROR AL ACTUALIZAR EL STOCK DE ROEL-CONTROL '.$errors, [], 'Admin.Payment.Notification');
                                    PrestaShopLogger::addLog($error, 4, '0000002', 'Cart', (int) ($order->id_cart));
                                    mysqli_close($con);
                                    die(Tools::displayError($error));
                                } 
