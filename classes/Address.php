...
class AddressCore extends ObjectModel
{
    ...
    public function add($autodate = true, $null_values = false)
    {
        ...
        $domicilio = $this->address1;
        $ciudad = $this->city;
        $id_customer = $this->id_customer;
        $rut = $this->dni;

        $query = "UPDATE clientes SET domicilio = UPPER('$domicilio'), ciudad = UPPER('$ciudad'), rut = UPPER('$rut') WHERE id_customer_tienda = $id_customer;";
        mysqli_query($con, $query);
        //MODIFICADO ALAN - ACTUALIZAR DIRECCION Y RUT EN EL SISTEMA FIN
