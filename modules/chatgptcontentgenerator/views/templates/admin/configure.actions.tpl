...
<script>
    $('.bootstrap .spin-offs_management').tooltip({
        html: true,
        title: alowSpinOffsManagment,
        trigger: 'click',
    });

    if (subscriptionExpiredAlertHtml) {
        $('#form-subscription-plan-used-limits .panel').append(subscriptionExpiredAlertHtml);
        $('#module-steps').prepend(subscriptionExpiredAlertHtml);

        if (!gptAutoOpenPlans) {
            ChatGptModal.displayRenewLimitsModal(subscriptionExpiredAlertHtml, ' ');
        }
    }
